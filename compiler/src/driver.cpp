...
std::vector<std::string> compilerArguments(const std::filesystem::path &generated,
    ...
    const std::vector<std::filesystem::path> runtimeSources{
        ...
        sdkAsset("runtime/src/process_stream_posix.c",
                 std::filesystem::path{FOUNDATION_RUNTIME_PROCESS_STREAM_POSIX_SOURCE}),
        sdkAsset("runtime/src/process_stream_windows.c",
                 std::filesystem::path{FOUNDATION_RUNTIME_PROCESS_STREAM_WINDOWS_SOURCE}),
    ...
    if (compilerId == "MSVC") {
        ...
        arguments.insert(arguments.end(), {"/nologo", "/std:c11",
                                           ...
                                           "/experimental:deterministic", "/W4", "/WX",
                                           "/Fo:" + objectDirectory,
...
bool compileLibraryObject(const std::filesystem::path &source,
    ...
    if (compilerId == "MSVC") {
        arguments.insert(arguments.end(), {"/nologo", "/std:c11",
                                           "/experimental:c11atomics",
                                           "/experimental:deterministic", "/O2", "/W4", "/WX",
