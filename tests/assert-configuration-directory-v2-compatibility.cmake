if(NOT DEFINED COMPILER OR NOT DEFINED GO_EXECUTABLE OR NOT DEFINED GIT_EXECUTABLE OR
   ...
   NOT DEFINED DIRECTORY OR NOT DEFINED EXPECTED OR NOT DEFINED WORK)
    message(FATAL_ERROR "configuration directory compatibility assertion is missing an input")
endif()
...
if(NOT revision_result EQUAL 0 OR NOT actual_revision STREQUAL v2_revision)
    message(FATAL_ERROR "Foundation v2 configuration directory revision mismatch: ${actual_revision}${revision_error}")
endif()
execute_process(
    COMMAND "${GIT_EXECUTABLE}" -C "${V2_SOURCE}" diff --quiet --
        core/configuration/source/dir go.mod
    RESULT_VARIABLE dirty_result
)
if(NOT dirty_result EQUAL 0)
    message(FATAL_ERROR "Foundation v2 configuration directory baseline has local changes")
endif()
...
set(foundation_executable "${WORK}/foundation-configuration-directory${EXECUTABLE_SUFFIX}")
execute_process(
...
if(NOT foundation_build_result EQUAL 0)
    message(FATAL_ERROR "cannot build Foundation configuration directory fixture:\n${foundation_build_output}${foundation_build_error}")
endif()
set(go_executable "${WORK}/go-configuration-directory${EXECUTABLE_SUFFIX}")
execute_process(
...
if(NOT go_build_result EQUAL 0)
    message(FATAL_ERROR "cannot build Foundation v2 configuration directory fixture:\n${go_build_output}${go_build_error}")
endif()
execute_process(
    COMMAND "${foundation_executable}" "${DIRECTORY}/alpha.json" "${DIRECTORY}/beta.json"
...
if(NOT foundation_result EQUAL 0)
    message(FATAL_ERROR "Foundation configuration directory fixture failed:\n${foundation_output}${foundation_error}")
endif()
execute_process(
    COMMAND "${go_executable}" "${DIRECTORY}"
    RESULT_VARIABLE go_result
    OUTPUT_VARIABLE go_output
    ERROR_VARIABLE go_error
)
if(NOT go_result EQUAL 0)
    message(FATAL_ERROR "Foundation v2 configuration directory fixture failed:\n${go_output}${go_error}")
endif()
...
if(NOT foundation_output STREQUAL expected_output OR NOT go_output STREQUAL expected_output OR
   NOT foundation_output STREQUAL go_output)
    message(FATAL_ERROR "Foundation configuration directory output differs from Foundation v2")
endif()
message(STATUS "Foundation and Foundation v2 configuration directory behavior matches ${v2_revision}")
