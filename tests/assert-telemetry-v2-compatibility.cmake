if(NOT DEFINED COMPILER OR NOT DEFINED GO_EXECUTABLE OR NOT DEFINED GIT_EXECUTABLE OR
   ...
   NOT DEFINED EXPECTED OR NOT DEFINED WORK)
    message(FATAL_ERROR "telemetry compatibility assertion is missing an input")
endif()
...
execute_process(
    COMMAND "${GIT_EXECUTABLE}" -C "${V2_SOURCE}" diff --quiet --
        core/telemetry go.mod
    RESULT_VARIABLE dirty_result
)
if(NOT dirty_result EQUAL 0)
    message(FATAL_ERROR "Foundation v2 telemetry baseline has local changes")
endif()
...
set(foundation_executable "${WORK}/foundation-telemetry${EXECUTABLE_SUFFIX}")
execute_process(
...
if(NOT foundation_build_result EQUAL 0)
    message(FATAL_ERROR
        "cannot build Foundation telemetry fixture:\n${foundation_build_output}${foundation_build_error}")
endif()

set(go_executable "${WORK}/go-telemetry${EXECUTABLE_SUFFIX}")
execute_process(
...
if(NOT foundation_result EQUAL 0)
    message(FATAL_ERROR
        "Foundation telemetry fixture failed:\n${foundation_output}${foundation_error}")
endif()
...
if(NOT foundation_output STREQUAL expected_output)
    message(FATAL_ERROR
        "Foundation telemetry output mismatch:\nexpected:\n${expected_output}actual:\n${foundation_output}")
endif()
if(NOT go_output STREQUAL expected_output)
    message(FATAL_ERROR
        "Foundation v2 telemetry output mismatch:\nexpected:\n${expected_output}actual:\n${go_output}")
endif()
if(NOT foundation_output STREQUAL go_output)
    message(FATAL_ERROR "Foundation and Foundation v2 telemetry outputs differ")
endif()

message(STATUS "Foundation and Foundation v2 telemetry behavior matches ${v2_revision}")
