if(NOT DEFINED COMPILER OR NOT DEFINED GO_EXECUTABLE OR NOT DEFINED GIT_EXECUTABLE OR
   ...
   NOT DEFINED CASES OR NOT DEFINED EXPECTED OR NOT DEFINED WORK)
    message(FATAL_ERROR "validation compatibility assertion is missing an input")
endif()
...
execute_process(
    COMMAND "${GIT_EXECUTABLE}" -C "${V2_SOURCE}" diff --quiet --
        core/validation core/tags go.mod
    RESULT_VARIABLE dirty_result
)
if(NOT dirty_result EQUAL 0)
    message(FATAL_ERROR "Foundation v2 validation baseline has local changes")
endif()
...
set(foundation_executable "${WORK}/foundation-validation${EXECUTABLE_SUFFIX}")
execute_process(
...
if(NOT foundation_build_result EQUAL 0)
    message(FATAL_ERROR
        "cannot build Foundation validation fixture:\n${foundation_build_output}${foundation_build_error}")
endif()

set(go_executable "${WORK}/go-validation${EXECUTABLE_SUFFIX}")
execute_process(
...
if(NOT go_build_result EQUAL 0)
    message(FATAL_ERROR "cannot build Foundation v2 fixture:\n${go_build_output}${go_build_error}")
endif()

execute_process(
    COMMAND "${foundation_executable}" "${CASES}"
...
if(NOT foundation_result EQUAL 0)
    message(FATAL_ERROR
        "Foundation validation fixture failed:\n${foundation_output}${foundation_error}")
endif()

execute_process(
    COMMAND "${go_executable}" "${CASES}"
    RESULT_VARIABLE go_result
    OUTPUT_VARIABLE go_output
    ERROR_VARIABLE go_error
)
if(NOT go_result EQUAL 0)
    message(FATAL_ERROR "Foundation v2 fixture failed:\n${go_output}${go_error}")
endif()

file(READ "${EXPECTED}" expected_output)
if(NOT foundation_output STREQUAL expected_output)
    message(FATAL_ERROR
        "Foundation validation output mismatch:\nexpected:\n${expected_output}actual:\n${foundation_output}")
endif()
if(NOT go_output STREQUAL expected_output)
    message(FATAL_ERROR
        "Foundation v2 validation output mismatch:\nexpected:\n${expected_output}actual:\n${go_output}")
endif()
if(NOT foundation_output STREQUAL go_output)
    message(FATAL_ERROR "Foundation and Foundation v2 validation outputs differ")
endif()

message(STATUS "Foundation and Foundation v2 validation behavior matches ${v2_revision}")
