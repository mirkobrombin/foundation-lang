if(NOT DEFINED COMPILER OR NOT DEFINED GO_EXECUTABLE OR NOT DEFINED GIT_EXECUTABLE OR
   ...
   NOT DEFINED EXPECTED OR NOT DEFINED WORK)
    message(FATAL_ERROR "httpx compatibility assertion is missing an input")
endif()
...
execute_process(
    COMMAND "${GIT_EXECUTABLE}" -C "${V2_SOURCE}" diff --quiet --
        core/httpx core/resiliency go.mod
    RESULT_VARIABLE dirty_result
)
if(NOT dirty_result EQUAL 0)
    message(FATAL_ERROR "Foundation v2 HTTP baseline has local changes")
endif()
...
set(foundation_executable "${WORK}/foundation-httpx${EXECUTABLE_SUFFIX}")
execute_process(
...
if(NOT foundation_build_result EQUAL 0)
    message(FATAL_ERROR
        "cannot build Foundation HTTP fixture:\n${foundation_build_output}${foundation_build_error}")
endif()

set(go_executable "${WORK}/go-httpx${EXECUTABLE_SUFFIX}")
execute_process(
...
if(NOT foundation_output STREQUAL expected_output)
    message(FATAL_ERROR
        "Foundation HTTP output mismatch:\nexpected:\n${expected_output}actual:\n${foundation_output}")
endif()
if(NOT go_output STREQUAL expected_output)
    message(FATAL_ERROR
        "Foundation v2 HTTP output mismatch:\nexpected:\n${expected_output}actual:\n${go_output}")
endif()
if(NOT foundation_output STREQUAL go_output)
    message(FATAL_ERROR "Foundation and Foundation v2 HTTP outputs differ")
endif()

message(STATUS "Foundation and Foundation v2 HTTP behavior matches ${v2_revision}")
